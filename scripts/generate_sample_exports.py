...
Writes to ``data/inbox/`` (gitignored — the drop folder real exports will land in):
  ...
  _ground_truth.json               the DGP's true per-channel incremental key
                                   events — the answer key for mapping validation
...
platform-CLAIMED conversions — platforms over-claim by design here) so the mapping +
reporting work can be validated the same way the MMM is.
...
""", encoding="utf-8")
    ...
    print(f"Wrote 4 export files + ground truth + README to {INBOX}")
